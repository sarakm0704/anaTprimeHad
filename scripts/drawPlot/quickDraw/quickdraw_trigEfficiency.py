...
#        hep.histplot([h_eff_hltall,h_eff_hltall_dat], binning, ax=ax, histtype='errorbar', color=myc, capsize=2, xerr=True, yerr=y_errs, label=["Current all OR MC","Current all OR Data (SingleMu)"])
        hep.histplot([h_eff_hltcur,h_eff_hltcur_dat], binning, ax=ax, histtype='errorbar', color=myc, capsize=2, xerr=True, yerr=y_errs, label=["Current all OR MC","Current all OR Data (SingleMu)"])

        hep.histplot(h_eff_hltcur_dat/h_eff_hltcur, binning, yerr=np.sqrt(h_eff_hltall_dat)/h_eff_hltall, ax=rax, histtype='errorbar', color='k', capsize=2, label="Data/MC")
        # Final styling
        ...
        fig.savefig(f"{outloc}/sf_currentOR_{var}.png")
        fig.savefig(f"{outloc}/sf_currentOR_{var}.pdf")
        plt.close()
