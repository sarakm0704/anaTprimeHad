...
#myc = ['#5790FC','#016fb9','#67A239','#847E2B','#345325','#ef8a17','#D81200','#8029BD','#FF007B','#999999','#000000']
myc = ['#02ABBA','#016fb9','#67A239','#847E2B','#345325','#ef8a17','#D81200','#8029BD','#000000']
...
def cut_HLT1 (d):
    d = d.loc[(d['HLT_PFHT400_SixJet30_DoubleBTagCSV_p056']==1)]
    return d

def cut_HLT2 (d):
    d = d.loc[(d['HLT_PFHT450_SixJet40_BTagCSV_p056']==1)]
    return d

def cut_HLT3 (d):
    d = d.loc[(d['HLT_PFHT900']==1)]
    return d

def cut_HLT4 (d):
    d = d.loc[(d['HLT_PFHT750_4JetPt70']==1)]
    return d

def cut_HLT5 (d):
    d = d.loc[(d['HLT_PFHT750_4JetPt80']==1)]
    return d

def cut_HLT6 (d):
    d = d.loc[(d['HLT_PFHT800_4JetPt50']==1)]
    return d

def cut_HLT7 (d):
    d = d.loc[(d['HLT_PFHT850_4JetPt50']==1)]
    return d

def cut_HLT8 (d):
    d = d.loc[(d['HLT_PFJet450']==1)]
    return d
...
def cut_HLTcur (d):
    d = d.loc[ (d['HLT_PFHT400_SixJet30_DoubleBTagCSV_p056']==1) | (d['HLT_PFHT450_SixJet40_BTagCSV_p056']==1) | (d['HLT_PFHT900']==1) | (d['HLT_PFHT750_4JetPt70']==1) | (d['HLT_PFHT750_4JetPt80']==1) | (d['HLT_PFHT800_4JetPt50']==1) | (d['HLT_PFHT850_4JetPt50']==1) | (d['HLT_PFJet450']==1) ] 
    return d

def cut_HLTall (d):
    d = d.loc[ (d['HLT_PFHT400_SixJet30_DoubleBTagCSV_p056']==1) | (d['HLT_PFHT450_SixJet40_BTagCSV_p056']==1) | (d['HLT_PFHT900']==1) | (d['HLT_PFHT750_4JetPt70']==1) | (d['HLT_PFHT750_4JetPt80']==1) | (d['HLT_PFHT800_4JetPt50']==1) | (d['HLT_PFHT850_4JetPt50']==1) | (d['HLT_PFJet450']==1) | (d['HLT_PFJet500']==1)] 
    return d

for r in regions:
    print(f'running on region {r}')

#    loc = f'/Users/jieun/WORK/vlq/dnn/training/array/woHLT/Jet25/2016/{r}/'
    loc1 = f'/Users/jieun/WORK/vlq/dnn/training/array/woHLT/Jet25/2016/{r}/'
    loc2 = f'/Users/jieun/WORK/vlq/dnn/training/array/woHLT/Jet25/2016APV/{r}/'
    outloc = f'./output/2016/{r}'
    if not os.path.exists(f'{outloc}'): os.makedirs(f'{outloc}')
        
    # load dataset
    df1 = pd.read_hdf(f'{loc1}/array_SingleMuon.h5')
    df2 = pd.read_hdf(f'{loc2}/array_SingleMuon.h5')
...
#    input_var = ['goodht','jet1_pt','jet2_pt','jet3_pt','jet4_pt','jet5_pt','jet1_btag','jet2_btag','jet3_btag','mass_leadjets','mass_w','mass_h','mass_top','mass_wh','mass_secondtop','mass_tprime']
    for var in input_var:
        ...
        hep.style.use("CMS")
        fig, ax = plt.subplots()
        hep.cms.label("Preliminary", com = 13, lumi = 36.3, data = True, loc=0, ax=ax);
        ...
        h_whlt9, bins = np.histogram(df_hlt9[var], bins=infBins)
        ...
        hlt9_err = ratio_uncertainty(h_hlt9, h_hlt0, 'efficiency')
        hltcur_err = ratio_uncertainty(h_hltcur, h_hlt0, 'efficiency')
        hltall_err = ratio_uncertainty(h_hltall, h_hlt0, 'efficiency')

        #hlt_errs = [hlt1_err,hlt2_err,hlt3_err,hlt4_err,hlt5_err,hlt6_err,hlt7_err,hlt8_err,hlt9_err,hltcur_err,hltall_err]
        hlt_errs = [hlt1_err,hlt2_err,hlt3_err,hlt4_err,hlt5_err,hlt6_err,hlt7_err,hlt8_err,hltcur_err]

        # Main plot
        # standard
#        hep.histplot([h_eff_hlt1, h_eff_hlt2, h_eff_hlt3, h_eff_hlt4, h_eff_hlt5, h_eff_hlt6, h_eff_hlt7, h_eff_hlt8, h_eff_hlt9, h_eff_hltcur, h_eff_hltall], binning, ax=ax, histtype='errorbar', color=myc, capsize=2, xerr=True, yerr=hlt_errs, label=["HLT_PFHT400_SixJet30_DoubleBTagCSV_p056","HLT_PFHT450_SixJet40_BTagCSV_p056","HLT_PFHT900","HLT_PFHT750_4JetPt70","HLT_PFHT750_4JetPt80","HLT_PFHT800_4JetPt50","HLT_PFHT850_4JetPt50","HLT_PFJet450","HLT_PFJet500","Previous (B2G-19-001) all OR","all possible OR"])
        hep.histplot([h_eff_hlt1, h_eff_hlt2, h_eff_hlt3, h_eff_hlt4, h_eff_hlt5, h_eff_hlt6, h_eff_hlt7, h_eff_hlt8, h_eff_hltcur], binning, ax=ax, histtype='errorbar', color=myc, capsize=2, xerr=True, yerr=hlt_errs, label=["HLT_PFHT400_SixJet30_DoubleBTagCSV_p056","HLT_PFHT450_SixJet40_BTagCSV_p056","HLT_PFHT900","HLT_PFHT750_4JetPt70","HLT_PFHT750_4JetPt80","HLT_PFHT800_4JetPt50","HLT_PFHT850_4JetPt50","HLT_PFJet450","OR of all Triggers"])
